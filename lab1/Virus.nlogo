...
to setup-agents  ;;  Установите начальное число агентов и их начальные состояния
...
to disease-check ;;  Попытка заражения
  ask healthy[
...
to infect
  ...
  if immune-check < INFECTIOUSNESS
...
to live-or-die
  set recovery-check random 100
  ifelse recovery-check < CHANCE-RECOVERY
  [recover]
  [terminate]
end
...
to do-plots
  set-current-plot "Насиление"
  set-current-plot-pen "Здоровые"
  plot total-healthy
  set-current-plot-pen "Зараженные"
  plot total-infected
  set-current-plot-pen "Имунированые"
  plot total-immune
end
...
"" ""
PENS
"Здоровые" 1.0 0 -14439633 true "" ""
"Зараженные" 1.0 0 -2674135 true "" ""
